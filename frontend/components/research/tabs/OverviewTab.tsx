...
export function OverviewTab({ data, ticker }: Props) {
  ...
  const modelLabel =
    valuation.model_used === "fcff" ? "FCFF Intrinsic Value" :
    valuation.model_used === "ri" ? "Residual Income Value" :
    valuation.model_used === "excess_return" ? "Excess Return Value" :
    valuation.model_used === "ev_ebitda" ? "EV/EBITDA Implied Price" :
    "Intrinsic Value"
        ...
        <CardContent>
          <div className="grid gap-4 sm:grid-cols-3">
            <div>
              ...
              {valuation.diverges_materially ? (
                <p className="mt-0.5 text-xs text-amber-400/80 leading-snug truncate" title={valuation.route_reason || ""}>
                  {valuation.route_reason || "Model target price diverges — see details"}
                </p>
              ) : (

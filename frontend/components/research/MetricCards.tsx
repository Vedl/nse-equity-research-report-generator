...
export function MetricCards({ data }: Props) {
  ...
  const divergesMaterially = valuation.diverges_materially ?? false
  const upside = calcUpside(intrinsicValue, price.current)

  // Valuation card adapts based on diverges_materially flag.
        ...
        content: (
          <div className="mt-1">
            ...
            <Tooltip>
              ...
              >
                  Model diverges from market →{" "}
                  <span className="underline decoration-dotted">details</span>
              </TooltipTrigger>
              <TooltipContent className="max-w-64 text-xs leading-relaxed">
                {valuation.route_reason || "The selected valuation model target price diverges significantly from the current market price."}
              </TooltipContent>
            </Tooltip>
          </div>
        ),
        ...
        <div className="mt-1 flex flex-col gap-0.5">
          <div className="flex items-center gap-1.5">
            ...
            <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${
              valuation.model_used === "fcff" ? "bg-sky-500/10 text-sky-400" :
              valuation.model_used === "ri" ? "bg-indigo-500/10 text-indigo-400" :
              valuation.model_used === "excess_return" ? "bg-emerald-500/10 text-emerald-400" :
              "bg-amber-500/10 text-amber-400"
            }`}>
              {valuation.model_used === "fcff" ? "FCFF" :
               valuation.model_used === "ri" ? "RI" :
               valuation.model_used === "excess_return" ? "Excess" :
               "EV/EBITDA"}
            </span>
          </div>
          {valuation.relative?.low != null && valuation.relative?.high != null ? (
            <p className="text-[10px] text-muted-foreground truncate" title={`Comps range: ${fmtINR(valuation.relative.low)} - ${fmtINR(valuation.relative.high)}`}>
              Comps: {fmtINR(valuation.relative.low)} - {fmtINR(valuation.relative.high)}
            </p>
          ) : (
            <p className="text-[10px] text-muted-foreground">No peers available</p>
          )}
        </div>
      ),
    },

...
from equity_research.analysis.dcf import size_premium, two_stage_growth_schedule
...
def test_compute_wacc_known_answer(mini_profile, mini_income, mini_balance, cfg) -> None:
    ...
    # Ke = Rf + 1.2 × ERP + size premium (mcap=1000 → small-cap band, +2.0%)
...
def test_run_dcf_projected_fcff_length_equals_horizon(
    ...
    result = run_dcf(synthetic_profile, financials, cfg)
    # Two-stage DCF: Stage 1 (horizon) + Stage 2 linear fade years
    assert len(result.projected_fcff) == (
        cfg.dcf.projection_horizon + cfg.dcf.stage2_fade_years
    )
...
def test_ev_ebitda_valuation() -> None:
    # Implied EV = 12 * 200 = 2400
    # Implied Equity = 2400 - 400 = 2000
    # Price = 2000 / 10 = 200
    assert ev_ebitda_valuation(200.0, 400.0, 10.0, 12.0) == pytest.approx(200.0)
    assert ev_ebitda_valuation(None, 400.0, 10.0, 12.0) is None
    assert ev_ebitda_valuation(200.0, 400.0, 0.0, 12.0) is None
    # negative equity floored at 0
    # Implied EV = 1 * 200 = 200
    # Implied Equity = 200 - 400 = -200 -> floored at 0
    assert ev_ebitda_valuation(200.0, 400.0, 10.0, 1.0) == pytest.approx(0.0)

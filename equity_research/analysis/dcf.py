...
class WACCComponents:
    ...
    cost_of_equity: float         # Ke = Rf + β × ERP + α_size
    ...
    wacc: float
    size_premium: float = 0.0     # α_size: 0.5% large / 1.0% mid / 2.0% small cap
    beta_used: float = 1.0        # the beta actually applied in CAPM
...
class DCFResult:
    ...
    # Two-stage detail + scenario band (bear/base/bull)
    growth_schedule: list[float] = field(default_factory=list)  # per-year growth
    bear_value: float | None = None   # WACC +100bps, terminal g −50bps
    bull_value: float | None = None   # WACC −100bps, terminal g +50bps
...
def two_stage_growth_schedule(
    g_stage1: float,
    g_terminal: float,
    stage1_years: int = 5,
    fade_years: int = 5,
) -> list[float]:
    """Two-stage growth schedule with linear fade to terminal growth.

    Stage 1 (years 1..stage1_years): constant g_stage1.
    Stage 2 (fade): g_t = g_terminal + (g_stage1 − g_terminal) × (T − t) / fade_years
    where T = stage1_years + fade_years, so growth reaches g_terminal in the
    final explicit year.

    # CFA L2 Equity, Reading 23: Discounted Dividend Valuation (multi-stage
    # fade pattern); applied here to FCFF growth.
    """
    schedule = [g_stage1] * stage1_years
    total = stage1_years + fade_years
    for t in range(stage1_years + 1, total + 1):
        schedule.append(g_terminal + (g_stage1 - g_terminal) * (total - t) / fade_years)
    return schedule


def size_premium(market_cap_inr: float | None) -> float:
    """Size premium α_size added to CAPM cost of equity.

    Bands approximate the SEBI large/mid/small-cap boundaries by market cap:
    ≥ ₹50,000 Cr large (+0.5%), ≥ ₹17,000 Cr mid (+1.0%), below small (+2.0%).
    # Banz (1981) size effect; premia per the project valuation policy.
    """
    if market_cap_inr is None or market_cap_inr <= 0:
        return 0.01   # unknown size — apply mid-cap premium, conservative
    if market_cap_inr >= 50_000e7:
        return 0.005
    if market_cap_inr >= 17_000e7:
        return 0.010
    return 0.020
...
def compute_wacc(
    ...
    # Capital structure: 3-year average debt to avoid point-in-time distortion
    # # CFA L2 Corporate Issuers: target capital structure proxied by multi-year average
    debt_s = _col(balance, "total_debt").dropna()
    if not debt_s.empty:
        total_debt = max(0.0, float(debt_s.tail(3).mean()))
    else:
        total_debt = 0.0

    # Cost of equity — CAPM with size premium
    # # CFA L2 Equity, Reading 21: Return Concepts — Ke = Rf + β×ERP + α_size
    beta = profile.get("beta")
    if beta is None or (isinstance(beta, float) and math.isnan(beta)):
        beta = 1.0
        logger.warning("Beta missing — defaulting to 1.0 for CAPM")
    alpha_size = size_premium(float(market_cap))
    ke = (
        config.market.risk_free_rate
        + float(beta) * config.market.equity_risk_premium
        + alpha_size
    )
...
def run_dcf(
    ...
    # Two-stage schedule: Stage 1 at g1, then linear fade to terminal growth.
    fade_years = getattr(config.dcf, "stage2_fade_years", 0) or 0
    if fade_years > 0:
        growth_rates = two_stage_growth_schedule(
            g_stage1=growth_rates[0],
            g_terminal=config.dcf.terminal_growth_rate,
            stage1_years=config.dcf.projection_horizon,
    ...
    g_terminal = config.dcf.terminal_growth_rate
    if wacc <= g_terminal:
        # Never produce a negative terminal value — surface a descriptive error.
        from equity_research.utils.contracts import ValuationError
        raise ValuationError(
            f"WACC ({wacc:.2%}) ≤ terminal growth ({g_terminal:.2%}) — the Gordon "
            "growth terminal value is undefined. Review beta, ERP, or terminal "
            "growth assumptions before re-running the DCF."
        )
    ...
    # Scenario band: bear = WACC +100bps / g −50bps; bull = WACC −100bps / g +50bps
    def _scenario(w: float, g: float) -> float | None:
        if w <= g:
            return None
        try:
            tv_s = terminal_value(projected[-1], w, g)
            return dcf_equity_per_share(projected, tv_s, w, net_debt, shares)
        except (ValueError, ZeroDivisionError):
            return None

    bear_value = _scenario(wacc + 0.01, g_terminal - 0.005)
    bull_value = _scenario(wacc - 0.01, g_terminal + 0.005)
    ...
    return DCFResult(
        ...
        growth_schedule=list(growth_rates),
...
def ev_ebitda_valuation(
    ...
    """Compute implied equity value per share from EV/EBITDA peer multiple.

    Implied EV = peer_multiple * EBITDA
    Implied Equity Value = Implied EV - net_debt
    Implied Price = Implied Equity Value / shares
    """
    if ebitda is None or net_debt is None or shares is None or peer_multiple is None or shares <= 0:
        return None
    implied_ev = peer_multiple * ebitda
    implied_equity = implied_ev - net_debt
    return max(0.0, implied_equity / shares)

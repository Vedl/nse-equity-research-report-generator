...
# Plausibility ceilings per multiple — yfinance occasionally returns currency-
# mismatched EV fields (e.g. EV in INR over EBITDA in USD), producing absurd
# values like EV/EBITDA = 1069×. Out-of-range multiples are treated as missing
# (logged), never displayed or used in medians.
_MULTIPLE_CEILINGS = {"pe": 150.0, "ev_ebitda": 100.0, "pb": 50.0, "ev_sales": 50.0}


def _extract_peer_multiples(peer_profile: dict) -> PeerMultiples:
    """Pull trading multiples out of a normalized profile dict."""

    def _pos(val: object, kind: str) -> float | None:
        if val is None:
            return None
        try:
        ...
        if not (math.isfinite(f) and f > 0):
            return None
        if f > _MULTIPLE_CEILINGS[kind]:
            logger.warning(
                "Peer %s: implausible %s = %.1fx (ceiling %.0fx) — treated as missing",
                peer_profile.get("ticker", "?"), kind, f, _MULTIPLE_CEILINGS[kind],
            )
            return None
        return f
    ...
    return PeerMultiples(
        ...
        pe=_pos(peer_profile.get("trailing_pe"), "pe"),
        ev_ebitda=_pos(peer_profile.get("enterprise_to_ebitda"), "ev_ebitda"),
        pb=_pos(peer_profile.get("price_to_book"), "pb"),
        ev_sales=_pos(peer_profile.get("enterprise_to_revenue"), "ev_sales"),
    )

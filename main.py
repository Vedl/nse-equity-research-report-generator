...
def _build_research(ticker_ns: str) -> dict:
    ...
    if val_result.financial_result is not None:
        ...
        financial_out = {
            ...
            # Intrinsic IS the ROTE-based justified-P/B value (no dividend-only
            # DDM blend; pb_model_value == intrinsic_value).
            ...
            "roe":                   _clean(fin.roe),
            "roe_basis":             fin.roe_basis,
            "trailing_roe":          _clean(fin.trailing_roe),
            "forward_roe":           _clean(fin.forward_roe),
            "forward_eps":           _clean(fin.forward_eps),
            "projected_tangible_book_per_share": _clean(fin.projected_tangible_book_per_share),
            "valuation_method":      fin.valuation_method,
            "single_stage_value":    _clean(fin.single_stage_value),
            "explicit_years":        fin.explicit_years,
            "fade_years":            fin.fade_years,
            "terminal_rote":         _clean(fin.terminal_rote),
            "terminal_value_share":  _clean(fin.terminal_value_share),
            "implied_exit_ptbv":     _clean(fin.implied_exit_ptbv),
            "implied_ptbv":          _clean(fin.implied_ptbv),
    ...
    if nar is not None:
        narrative_out = {
            ...
            "consensus_cross_check":      nar.consensus_cross_check,
            "thesis":                     nar.thesis,
    ...
    return {
        ...
        "valuation": {
            ...
            "broker_target_median": _clean(val_result.broker_target_median),
            "broker_target_low":    _clean(val_result.broker_target_low),
            "broker_target_high":   _clean(val_result.broker_target_high),
